...
export default async (parent, args, context, info) => {
  ...
  const usersNotChildrenOrPartnerWithCurrentUser = await context.prisma.user.findMany({
    ...filters,
    where: {
      AND: [
        { coupleID: { not: { equals: userID } } },
        { parents: { every: { id: { not: { equals: userID } } } } }
      ],
      ...filters.where
    },
  ...
  return usersNotChildrenOrPartnerWithCurrentUser.filter(({ id }) => id !== requestedUser.id)
}
